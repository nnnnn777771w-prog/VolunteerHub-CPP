...
// ============================================================================
// ВНЕДРЕНИЕ ООП: КЛАСС УПРАВЛЕНИЯ БАЗОЙ ДАННЫХ ШТАБА
// ============================================================================
...
public:
    ...
    void loginSystem() {
        while (true) {
            ...
            colorCyan();
            cout << "=========================================\n";
            cout << "      АНАЛИЗ ДАННЫХ И ВХОД В VOLUNTEER HUB\n";
            cout << "=========================================\n";
            colorReset();
...
int main() {
    ...
    while (choice != 0) {
        ...
        cout << "   Текущий режим: ";
        if (hub.getAdmin()) { hub.colorGreen(); cout << "[КООРДИНАТОР]\n"; }
        else { hub.colorYellow(); cout << "[ГОСТЬ]\n"; }
        hub.colorCyan();
        cout << "=========================================\n";
        hub.colorReset();

        cout << "1. Регистрация волонтеров и заявок ЧС "; if (!hub.getAdmin()) { hub.colorRed(); cout << "(ЗАБЛОКИРОВАНО)"; hub.colorReset(); } cout << "\n";
        cout << "2. Экстренный мэтчинг (Поиск кадров) "; if (!hub.getAdmin()) { hub.colorRed(); cout << "(ЗАБЛОКИРОВАНО)"; hub.colorReset(); } cout << "\n";
        cout << "3. Фиксация помощи и начисление часов "; if (!hub.getAdmin()) { hub.colorRed(); cout << "(ЗАБЛОКИРОВАНО)"; hub.colorReset(); } cout << "\n";
        cout << "4. Показать заявки (Сортировка по ЧС)\n";
        cout << "5. Вывести аналитику эффективности штаба\n";
        cout << "6. Экспорт официального отчета в файл "; if (!hub.getAdmin()) { hub.colorRed(); cout << "(ЗАБЛОКИРОВАНО)"; hub.colorReset(); } cout << "\n";
        ...
        choice = hub.getSafeIntInput();

        switch (choice) {
        case 1:
            if (hub.getAdmin()) hub.registerEntity();
            else { hub.colorRed(); cout << " [!] Доступ закрыт! Гости не могут менять базу данных.\n"; hub.colorReset(); hub.waitForEnter(); }
            break;
        case 2:
            if (hub.getAdmin()) hub.emergencyMatching();
            else { hub.colorRed(); cout << " [!] Доступ закрыт! Поиск доступен только координаторам.\n"; hub.colorReset(); hub.waitForEnter(); }
            break;
        case 3:
            if (hub.getAdmin()) hub.closeRequestAndReward();
            else { hub.colorRed(); cout << " [!] Доступ закрыт! Начислять опыт может только админ.\n"; hub.colorReset(); hub.waitForEnter(); }
            break;
        case 4: hub.printSortedRequests(); break;
        case 5: hub.showAnalytics(); break;
        case 6:
            if (hub.getAdmin()) hub.exportReport();
            else { hub.colorRed(); cout << " [!] Доступ закрыт! Экспорт отчетов доступен только админу.\n"; hub.colorReset(); hub.waitForEnter(); }
            break;
        case 7: hub.showInstructions(); break;
        case 0:
            if (hub.getAdmin()) {
                hub.saveRequests();
                hub.saveVolunteers();
